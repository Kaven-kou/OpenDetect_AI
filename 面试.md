...
- **关键实现**：Web 路径入库前 `interrupt` 让用户勾选（HITL）；确定性 chunk ID（`arxiv_id__chunk_N`）保证**幂等**；写入后使 BM25 缓存失效；付费墙论文（无 arXiv ID、下载失败）直接放弃，可重试的最多 2 次（防死循环）。
...
- **关键实现**：无检索内容时不花 LLM 直接提示；**fail-open**——校验本身出错就放行原答案，不让校验服务故障拖垮主流程。
- **面试考点**：这是"你怎么防 RAG 幻觉"的直接答案，比再加一个总结 Agent 更有价值；fail-open 体现"质量增强层不该成为单点故障"。
...
分三种处理，都不让单点故障拖垮整体：
- **可降级的**（Verifier、Self-Query、Rerank、Resolve 改写）→ **fail-open**：出错就退回更简单的路径或放行原结果；
